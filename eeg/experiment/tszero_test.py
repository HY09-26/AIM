"""Time-segment masking — zero replacement (MoRF / LeRF)."""
import sys
import os
...
OUT_DIR  = os.path.join(SAVE_DIR, f"repeat{rep}/ts_test_zero/{dataname}_sto")
os.makedirs(OUT_DIR, exist_ok=True)

modes  = [('mo', -1), ('le', 1)]
leg    = ['gradient', 'gradientxinput', 'smoothgrad', 'smoothgrad_sq',
          'vargrad', 'inte_grad', 'random']
ts_dim = 11     # k = 1..11, each step masks k×5% of the time window
n_subs = len(cfg.sub_list)
filler = '' if args.abs_saliency else 'n'


def mask_timeseg_zero(model, xtest, labels, start_idx, wsize):
    """Zero out a time window [start_idx, start_idx+wsize), evaluate model."""
        ...
        for l in leg:
            ...
            for s in tqdm(range(n_subs)):
                ...
                for k in range(1, ts_dim + 1):
                    ...
                    loss, acc, out = mask_timeseg_zero(testmodel, xtest, ytest, max_idx, wsize)
                    hist['acc'][s][k-1]  = acc
                    hist['loss'][s][k-1] = loss
                    hist['out'][s][k-1]  = out

                    loss, acc, out = mask_timeseg_zero(testmodel, xtest, ytest, min_idx, wsize)
                    hist1['acc'][s][k-1]  = acc
