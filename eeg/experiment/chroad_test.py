"""Channel masking — spatial-interpolation (ROAD) replacement (MoRF / LeRF)."""
import sys
import os
...
from experiment_utils.masking_utils import NoisySpatialImputer, get_topology
...
topology = get_topology(dataname)
...
OUT_DIR  = os.path.join(SAVE_DIR, f"repeat{rep}/ch_test_road/{dataname}")
...
def mask_channels_road(model, xtest, chrank, labels):
    """Spatially interpolate ranked channels, evaluate model."""
    benigns  = xtest.squeeze().copy()
    mixtures = benigns.copy()
    for t in range(benigns.shape[0]):
        imputer = NoisySpatialImputer(
            list(chrank[t]), topology, noise=float(np.std(benigns[t]))
        )
        mixtures[t, chrank[t]] = imputer.impute(benigns[t])[chrank[t]]
        ...
        for l in leg:
            ...
            for s in tqdm(range(n_subs)):
                ...
                for k in range(1, n_ch + 1):
                    ...
                    loss, acc, out = mask_channels_road(testmodel, xtest, chrankM, ytest)
                    hist['acc'][s][k-1]  = acc
                    hist['loss'][s][k-1] = loss
                    hist['out'][s][k-1]  = out

                    loss, acc, out = mask_channels_road(testmodel, xtest, chrankL, ytest)
                    hist1['acc'][s][k-1]  = acc
