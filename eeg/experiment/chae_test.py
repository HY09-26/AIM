"""Channel masking — adversarial example (AE) replacement (MoRF / LeRF)."""
import sys
import os
...
OUT_DIR  = os.path.join(SAVE_DIR, f"repeat{rep}/ch_test_ae/{dataname}")
...
def mask_channels_ae(model, xtest, aes, chrank, labels):
    """Replace ranked channels with adversarial values, evaluate model."""
    benigns  = xtest.squeeze().copy()
    aes      = aes.squeeze()
    mixtures = benigns.copy()
    mixtures[np.arange(chrank.shape[0])[:, None], chrank] = \
        aes[np.arange(chrank.shape[0])[:, None], chrank]
        ...
        for l in leg:
            ...
            for s in tqdm(range(n_subs)):
                ...
                for k in range(1, n_ch + 1):
                    ...
                    loss, acc, out = mask_channels_ae(testmodel, xtest, aes, chrankM, ytest)
                    hist['acc'][s][k-1]  = acc
                    hist['loss'][s][k-1] = loss
                    hist['out'][s][k-1]  = out

                    loss, acc, out = mask_channels_ae(testmodel, xtest, aes, chrankL, ytest)
                    hist1['acc'][s][k-1]  = acc
