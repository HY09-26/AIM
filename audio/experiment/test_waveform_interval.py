# test_waveform_interval.py
# Interval-based MoRF/LeRF for waveform datasets:
# - AudioMNIST / ESC50 / MSOS
# - model: res1dnet31 | audionet
# - mask_type: zero | pgd | road
#
# Interval spec:
#   * masking schedule: length L_k = round((k/n_steps)*T), k=0..n_steps
#   * For each sample:
#       - MoRF: choose interval length L_k with MAX saliency-sum
#       - LeRF: choose interval length L_k with MIN saliency-sum
#   * zero baseline: 0.0
#   * PGD: compute once on full waveform; masked interval replaced by X_adv interval
#   * ROAD: points-only replacement; segment = the chosen interval; cache across k
#
# Notes:
...
# ============================================================
# Dataset loaders (keep dataset-specific logic)
# ============================================================
...
def eval_epoch(model, loader, model_name: str):
    ...
    for x, y in loader:
        ...
        out = model(adapt_wave_input(x, model_name))
...
# ============================================================
# PGD (compute ONCE on full waveform)
# ============================================================
...
# ============================================================
# Interval selection by sliding-window saliency sum
# ============================================================
def best_interval_by_sum(s: np.ndarray, L: int, mode: str):
    """
    s: (T,) saliency for ONE sample
    L: interval length
    mode: "morf" -> argmax window sum, "lerf" -> argmin window sum
    returns (t0, t1) half-open
    """
    T = s.shape[0]
    if L <= 0:
        return (0, 0)
    ...
    # window_sum[t] = sum(s[t:t+L]) for t=0..T-L
    # vectorize:
    w = pref[L:] - pref[:-L]  # (T-L+1,)
...
# ============================================================
# ROAD (same as your points-only robust segment, but here idx is contiguous)
# ============================================================
...
def road_apply_interval_points_only(
    x_work: np.ndarray,     # (T,) modified in-place
    x_orig: np.ndarray,     # (T,) reference
    t0: int,
    t1: int,
    seg_cache: dict,        # key=(t0,t1)->segment
    ...
    if key not in seg_cache:
        seg_cache[key] = road_generate_segment_robust(x_orig, int(t0), int(t1), rng)
    rep = seg_cache[key]
    if rep is None:
        seg_len = int(t1 - t0)
        mid = (t0 + t1) // 2
...
def run_morf_lerf_interval(
    ...
    # ROAD cache across k: per sample
    road_cache_per_sample = [dict() for _ in range(N)]
    rng = np.random.default_rng(road_seed)

    # schedule: L_k = round((k/n_steps)*T)
    L_list = [int(round(k * (T / max(1, n_steps)))) for k in range(n_steps + 1)]
    L_list = [max(0, min(L, T)) for L in L_list]
    ...
    for k, L in enumerate(L_list):
        ...
        if L > 0:
            for i in range(N):
                ...
                if mask_type == "zero":
                    ...
                    # shortest cover segment is exactly [t0,t1)
                    road_apply_interval_points_only(
                        x_work=x[i],
                        x_orig=X[i],
                        t0=t0,
                        t1=t1,
        ...
        print(
            f"[{mode.upper()}-INTERVAL] step {k:02d}/{n_steps} "
            f"| masked={100*k/n_steps:5.1f}% | L={L:5d}/{T} | acc={acc:.4f}"
        )
